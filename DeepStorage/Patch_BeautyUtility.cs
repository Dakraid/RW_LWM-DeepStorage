...
namespace LWM.DeepStorage
{
    ...
    {
        private static bool Prefix(ref float __result, IntVec3 c, Map map, List<Thing> countedThings = null)
        {
            ...
            if (countedThings != null)
            {
                ...
                {
                    __result = map.terrainGrid.TerrainAt(c).GetStatValueAbstract(StatDefOf.Beauty);
                    return false;
                }
            ...
            __result = storage.GetStatValue(StatDefOf.Beauty) +
                       map.terrainGrid.TerrainAt(c).GetStatValueAbstract(StatDefOf.Beauty);
            return false;
        }
    }
}