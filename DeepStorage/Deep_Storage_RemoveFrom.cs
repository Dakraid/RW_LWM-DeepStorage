...
namespace LWM.DeepStorage
{
    ...
    public static class Patch_StartCarryThing_Delegate
    {
        ...
        private static MethodBase TargetMethod() //The target method is found using the custom logic defined here
        {
            ...
            predicateClass = typeof(Toils_Haul).GetNestedTypes(AccessTools.all)
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (; i < code.Count - 1; i++)
            {
                ...
                else if (foundCorrectException && code[i].opcode == OpCodes.Throw)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Ldfld,
                        AccessTools.Field(predicateClass, "toil"));
            ...
            var cleanUp = new CodeInstruction(OpCodes.Call, AccessTools
        ...
        public static void FillThisStackIfAble(Toil toil, Thing thing, int carryCapacity)
        {
            ...
            var num = Mathf.Min(carryCapacity, toil.actor.jobs.curJob.count);
            ...
            if (slotGroup == null || !(slotGroup?.parent is ThingWithComps) ||
                ((ThingWithComps) slotGroup.parent).TryGetComp<CompDeepStorage>() == null) //Giant piles on the ground can be messy, so I am okay with
        ...
        {
            if (tmpThing == null) return; // only do this if the pawn in messing around in Deep Storage
