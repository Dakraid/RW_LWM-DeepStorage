...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public static void TidyStacksOf(Thing thing)
        {
                ...
                || thing.Position == IntVec3.Invalid)
            {
                // just in case
#if DEBUG
                if (showDebug[(int) DBF.TidyStacksOf])
                {
            ...
            {
                if (thingStacks[i] == null || thingStacks[i].Destroyed) break;
                ...
                if (thingStacks[i].stackCount > stackLimit)
                {
                    ...
                    Log.Warning("LWM.DeepStorage found " + thingStacks[i].stackCount + " of " +
                                thingStacks[i] + " when the stackLimit is " +
                ...
                {
                    if (thingStacks[j] == null) continue;
                ...
                if (thingStacks[i].stackCount < stackLimit) break; // we can't clean up any more...continue looking for unfull stacks (probably this one)
